...
// Because nothing says "hire me" like admitting my mediocrity upfront
const TypewriterText = () => {
  ...
  useEffect(() => {
    ...
    if (isTyping) {
      if (text.length < currentPhrase.length) {
        ...
        }, 80 + Math.random() * 50); // Slightly random typing speed for authenticity
  ...
  // Calculate width based on longest phrase
...
const HeroSection = () => {
  ...
  // Toggle between light (white background, black text) and dark (black background, white text)
  const toggleTheme = () => {
    setIsDarkMode(!isDarkMode);
  };

  // Determine container classes for only black and white
  const containerClasses = isDarkMode
    ? 'bg-black text-white'
    : 'bg-white text-black';

  return (
    <section className={`min-h-screen flex items-center justify-center py-16 px-4 ${containerClasses} overflow-hidden`}>
      <div className="container mx-auto max-w-6xl relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            ...
            {/* CTA button that probably links to more disappointment */}
            <div className="mt-10">
              <a
                href="/#projects"
                onClick={(e) => {
